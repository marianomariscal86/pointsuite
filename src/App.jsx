...
// Clasificación extendida: incluye Promoción Prepago para clientes al corriente con <365 días a renovación
function cDelEx(bal,dOvr){if(dOvr>0)return cDel(dOvr);if(bal===0&&daysToNextRenewal()<365)return{l:"Promoción Prepago",c:"#10b981",r:0};return cDel(dOvr);}
...
const INIT_U=[
 {id:1,username:"superadmin",password:"super123",role:"superadmin",name:"Super Administrador",color:B,salary:0,commissions:{collection:0},active:true},
 {id:2,username:"admin1",password:"admin123",role:"admin",name:"Carmen Ríos",color:Y,salary:5000,commissions:{collection:0.015},active:true},
 {id:3,username:"cajero1",password:"cajero123",role:"cajero",name:"Roberto Sanz",color:G,salary:2500,commissions:{collection:0.008},active:true},
 {id:4,username:"gestor1",password:"gestor123",role:"gestor",name:"Ana López",color:P,salary:3000,commissions:{collection:0.02},active:true},
 {id:5,username:"gestor2",password:"gestor456",role:"gestor",name:"Luis Mora",color:C,salary:3000,commissions:{collection:0.02},active:true},
];
...
function SearchBox({clients,onSelect,selectedId}){
 ...
 {found.map(c=>{const d=dOvr(c.dueDate);const cl=cDelEx(c.balance,d);return(
...
function Login({onLogin,users}){
 ...
 const go=()=>{setLd(true);setTimeout(()=>{const f=users.find(x=>x.username===u&&x.password===p&&x.active!==false);if(f)onLogin(f);else{setErr("Credenciales incorrectas");setLd(false);}},400);};
 return(<div style={{minHeight:"100vh",background:BG,fontFamily:"'DM Mono','Fira Code','Courier New',monospace",display:"flex",alignItems:"center",justifyContent:"center"}}>
 <div style={{position:"fixed",inset:0,backgroundImage:`radial-gradient(${BG3} 1px,transparent 1px)`,backgroundSize:"26px 26px",opacity:.5}}/>
 <div style={{position:"relative",width:"100%",maxWidth:420,padding:14}}>
 <div style={{textAlign:"center",marginBottom:24}}>
 <div style={{width:50,height:50,background:"linear-gradient(135deg,#1d4ed8,#7c3aed)",borderRadius:13,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,margin:"0 auto 10px"}}>◈</div>
 <div style={{color:T1,fontWeight:800,fontSize:17,letterSpacing:".07em"}}>POINTSUITE</div>
 <div style={{color:T4,fontSize:9,letterSpacing:".12em"}}>OWNERSHIP MANAGEMENT SYSTEM</div>
 </div>
 <div style={{background:BG2,border:`1px solid ${BD}`,borderRadius:14,padding:24}}>
 {[["Usuario",u,setU,"text"],["Contraseña",p,setP,"password"]].map(([l,v,s,t])=>(
 <div key={l} style={{marginBottom:12}}><label style={{fontSize:9,color:T4,letterSpacing:".1em",textTransform:"uppercase",display:"block",marginBottom:4}}>{l}</label>
 <input type={t} value={v} onChange={e=>{s(e.target.value);setErr("");}} onKeyDown={e=>e.key==="Enter"&&go()} style={IS}/></div>
 ))}
 {err&&<div style={{color:R,fontSize:11,marginBottom:8,textAlign:"center"}}>⚠ {err}</div>}
 <button onClick={go} disabled={ld} style={{width:"100%",padding:"10px",background:ld?"#152040":IND,color:"#fff",border:"none",borderRadius:8,fontSize:13,fontWeight:700,cursor:ld?"default":"pointer",fontFamily:"inherit"}}>{ld?"Verificando…":"Entrar →"}</button>
<div style={{marginTop:14,padding:"9px",background:BG,borderRadius:7,border:`1px solid ${T5}`}}>
 <div style={{fontSize:8,color:T4,letterSpacing:".1em",textTransform:"uppercase",marginBottom:6}}>Demo — clic para autocompletar</div>
 {users.filter(u=>u.active!==false).map(u=>(<div key={u.id} onClick={()=>{setU(u.username);setP(u.password);setErr("");}} style={{display:"flex",justifyContent:"space-between",marginBottom:4,cursor:"pointer",padding:"2px 0",borderBottom:`1px solid ${T5}`}}>
 <span style={{fontSize:11,color:u.color,fontWeight:600}}>{u.username}</span>
 <span style={{fontSize:9,color:T4}}>{RM[u.role]?.l}</span>
 </div>))}
 </div>
 </div>
 </div>
...
// ── CLIENTS VIEW ─────────────────────────────────────────────────────────────
function ClientsView({clients,setClients,pp,cu,payments,reservations}){
...
function CashierView({clients,payments,setPayments,setClients,pp,pms,cu,users,pendingPayments,setPendingPayments,promises,setPromises,fxRate,setFxRate}){
 const isCajero=cu.role==="cajero";
 const isGestor=cu.role==="gestor";
 const [sid,setSid]=useState(null),[ptype,setPtype]=useState("maintenance"),[mantAmt,setMantAmt]=useState(""),[moraAmt,setMoraAmt]=useState(""),[mantDisc,setMantDisc]=useState(0),[moraDisc,setMoraDisc]=useState(0),[note,setNote]=useState(""),[ok,setOk]=useState(null);
 ...
 // Pending del cliente: es lo que el gestor propone registrar
 const mantAmtN=+mantAmt||0,moraAmtN=+moraAmt||0;
 // Descuentos: max 20% mantenimiento, max 100% moratorios. Se aplican primero a moratorios y después a mantenimiento.
 const maxDiscMant=mantAmtN*0.20,maxDiscMora=moraAmtN*1.00;
 const mantDiscN=Math.min(Math.max(0,+mantDisc||0),maxDiscMant);
 const moraDiscN=Math.min(Math.max(0,+moraDisc||0),maxDiscMora);
 const totalDescuento=mantDiscN+moraDiscN;
 const totalACobrarMant=Math.max(0,mantAmtN-mantDiscN);
 const totalACobrarMora=Math.max(0,moraAmtN-moraDiscN);
 const totalACobrar=totalACobrarMant+totalACobrarMora;
 const submit=()=>{
  if(!client||(mantAmtN<=0&&moraAmtN<=0))return;
  const prop={id:Date.now(),clientId:client.id,clientName:client.name,contractNo:client.contractNo,
   ptype,mantAmt:mantAmtN,moraAmt:moraAmtN,mantDisc:mantDiscN,moraDisc:moraDiscN,
   totalACobrar,submittedBy:cu.username,submittedAt:new Date().toISOString(),
   note,status:"Por Validar"};
  setPendingPayments(ps=>[...ps,prop]);
  setOk({name:client.name,total:totalACobrar,id:prop.id});
  setSid(null);setMantAmt("");setMoraAmt("");setMantDisc(0);setMoraDisc(0);setNote("");
 };
 const validatePayment=(id,mid)=>{
  ...
  // Calcular pts acreditados si es mantenimiento o compra puntos
  // Calc pts (legacy vars unused, kept for backward compat)
  const ptsFinal=pp2.ptype==="maintenance"?Math.floor((pp2.totalACobrar-(pp2.moraAmt-pp2.moraDisc))/pp):ptsAcrBack;
  const payment={id:Date.now(),clientId:pp2.clientId,clientName:pp2.clientName,contractNo:pp2.contractNo,
 ...
 return(<div style={{display:"grid",gridTemplateColumns:isCajero?"1fr 1fr":"1fr 1.2fr",gap:14,alignItems:"start"}}>
  {!isCajero&&(<div style={{background:BG2,border:`1px solid ${BD}`,borderRadius:12,padding:18}}>
   ...
   {client&&<div style={{padding:"8px 10px",background:BG3,borderRadius:7,border:`1px solid ${cls.c}33`}}>
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:4,marginBottom:5}}>
     {[["Pts/Año",fP(client.annualPoints),P],["Pagados",fP(client.paidPoints),G],["Saldo",f$(client.balance),client.balance>0?R:G],["Mora",f$(interest),Y]].map(([l,v,c])=>(<div key={l}><div style={{fontSize:8,color:T4,textTransform:"uppercase"}}>{l}</div><div style={{color:c,fontWeight:700,fontSize:11}}>{v}</div></div>))}
    </div>
    <Bdg l={cls.l}/>
   </div>}
   <Inp label="Concepto" value={ptype} onChange={setPtype} opts={PT.map(t=>({v:t.v,l:t.l}))}/>
   <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
   <div><Inp label="Mantenimiento ($MXN)" value={mantAmt} onChange={setMantAmt} type="number"/>{mantAmtN>0&&fxRate?.rate>0&&<div style={{fontSize:9,color:T4,marginTop:2}}>≈ ${(mantAmtN/fxRate.rate).toFixed(2)} USD</div>}</div>
   <div><Inp label="Moratorios ($MXN)" value={moraAmt} onChange={setMoraAmt} type="number"/>{moraAmtN>0&&fxRate?.rate>0&&<div style={{fontSize:9,color:T4,marginTop:2}}>≈ ${(moraAmtN/fxRate.rate).toFixed(2)} USD</div>}</div>
   </div>
   <div style={{background:BG3,borderRadius:7,padding:"9px 11px",border:`1px solid ${Y}22`}}>
    <div style={{fontSize:9,color:Y,textTransform:"uppercase",letterSpacing:".08em",marginBottom:7}}>Descuentos autorizados</div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:6}}>
     <div><label style={{fontSize:8,color:T4,textTransform:"uppercase"}}>Descuento mant. (máx {f$(maxDiscMant)})</label><input type="number" value={mantDisc} onChange={e=>setMantDisc(+e.target.value)} max={maxDiscMant} style={{...IS,marginTop:2}}/></div>
     <div><label style={{fontSize:8,color:T4,textTransform:"uppercase"}}>Descuento mora (máx {f$(maxDiscMora)})</label><input type="number" value={moraDisc} onChange={e=>setMoraDisc(+e.target.value)} max={maxDiscMora} style={{...IS,marginTop:2}}/></div>
    </div>
    <div style={{fontSize:9,color:T4}}>Regla: máx 20% en mantenimiento, máx 100% en moratorios. Los descuentos se aplican <b>primero a moratorios</b> y después a mantenimiento.</div>
    {(mantDiscN>maxDiscMant||moraDiscN>maxDiscMora)&&<div style={{color:R,fontSize:10,marginTop:4}}>⚠ Descuento excede el máximo permitido.</div>}
   </div>
   <Inp label="Notas" value={note} onChange={setNote}/>
   </div>
   {mantAmtN+moraAmtN>0&&client&&<div style={{margin:"10px 0",padding:"10px",background:BG3,borderRadius:7}}>
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,marginBottom:6}}>
     {[["Subtotal",f$(mantAmtN+moraAmtN),T1],["Descuento",f$(totalDescuento),Y],["A cobrar",f$(totalACobrar),G],["Saldo result.",f$(Math.max(0,(client?.balance||0)-totalACobrarMant)),G]].map(([l,v,c])=>(<div key={l}><div style={{fontSize:8,color:T4,textTransform:"uppercase"}}>{l}</div><div style={{color:c,fontWeight:700,fontSize:11}}>{v}</div></div>))}
    </div>
   </div>}
   <Btn label="Enviar a Caja para Validación" onClick={submit} disabled={!client||(mantAmtN<=0&&moraAmtN<=0)||mantDiscN>maxDiscMant||moraDiscN>maxDiscMora}/>
...
function ReservationsView({clients,reservations,setReservations,units,setUnits,uts,cu,rc,payments}){
 const [vtab,setVtab]=useState("avail"),[modal,setModal]=useState(false),[confDoc,setConfDoc]=useState(null),[presel,setPresel]=useState(null),[filt,setFilt]=useState("All");
 const availC=useMemo(()=>{return clients.filter(c=>{if(c.contractStatus!=="Active")return false;if(dOvr(c.dueDate)>0)return false;const u=reservations.filter(r=>r.clientId===c.id&&r.status==="Confirmed").reduce((a,r)=>a+r.pointsUsed,0);const sv=(c.savedPoints||[]).filter(sp=>new Date(sp.expiryDate)>new Date()).reduce((a,sp)=>a+sp.points,0);return c.paidPoints-u+sv>0;}).map(c=>{const u=reservations.filter(r=>r.clientId===c.id&&r.status==="Confirmed").reduce((a,r)=>a+r.pointsUsed,0);const sv=(c.savedPoints||[]).filter(sp=>new Date(sp.expiryDate)>new Date()).reduce((a,sp)=>a+sp.points,0);return{...c,avail:c.paidPoints-u+sv,sv};});
 },[clients,reservations]);
 const save=res=>{const nr={...res,id:Date.now(),processedBy:cu.username};setReservations(rs=>[...rs,nr]);setUnits(us=>us.map(u=>u.id===res.unitId?{...u,occ:[...(u.occ||[]),{ci:res.checkIn,co:res.checkOut}]}:u));setModal(false);setConfDoc(nr);};
...
// ── COLLECTIONS ──────────────────────────────────────────────────────────────
function CollectionsView({clients,payments,pp,promises,setPromises,cu}){
 ...
 const costPct=revM>0?((costM/revM)*100).toFixed(1):"0.0";
 ...
 const addProm=()=>{if(!pMod||!pAmt||!pDate)return;setPromises(ps=>[...ps,{id:Date.now(),clientId:pMod.id,clientName:pMod.name,contractNo:pMod.contractNo,promiseDate:pDate,amount:+pAmt,note:pNote,status:"Pendiente",gestorUsername:cu.username,gestorName:cu.name,createdAt:TOD}]);setPMod(null);setPAmt("");setPNote("");};
 return(<div>
 {pMod&&<Modal title={`Promesa — ${pMod.name}`} onClose={()=>setPMod(null)}>
 <div style={{background:BG3,borderRadius:7,padding:"8px 10px",marginBottom:10,fontSize:11,color:T3}}>Saldo: <b style={{color:R}}>{f$(pMod.balance)}</b>·<Bdg l={cDel(dOvr(pMod.dueDate)).l}/></div>
 <div style={{display:"flex",flexDirection:"column",gap:9}}>
 <Inp label="Fecha prometida" type="date" value={pDate} onChange={setPDate}/>
 <Inp label="Monto prometido ($)" type="number" value={pAmt} onChange={setPAmt}/>
 <Inp label="Notas" value={pNote} onChange={setPNote}/>
 </div>
 <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:12}}><Btn label="Cancelar" variant="ghost" onClick={()=>setPMod(null)}/><Btn label="Registrar Promesa" onClick={addProm} disabled={!pAmt||!pDate}/></div>
 </Modal>}
 <div style={{display:"flex",gap:9,flexWrap:"wrap",marginBottom:13}}>
 <Kpi label="Cobrado Hoy" value={f$(collT)} accent={G}/>
 <Kpi label="Cobrado Mes" value={f$(collM)} accent={B}/>
 <Kpi label="Saldo Pendiente" value={f$(myC.reduce((a,c)=>a+c.balance,0))} accent={R}/>
 <Kpi label="Promesas Hoy" value={promT.length} sub={f$(promT.reduce((a,p)=>a+p.amount,0))} accent={Y} warn={promT.length>0}/>
 <Kpi label="Promesas Mes" value={promM.length} sub={f$(promM.reduce((a,p)=>a+p.amount,0))} accent={P}/>
 <Kpi label="Costo Cobranza" value={costPct+"%"} sub={isAdmin?"Global":cu.name} accent={O} warn={parseFloat(costPct)>20}/>
 </div>
 ...
 <Tbl cols={["Contrato","Cliente","Vence","Días","Clasificación","Saldo","Interés","Total","Cob%","Estado",""]}
 rows={fc.map(c=>{const cov=Math.round((c.paidPoints/c.annualPoints)*100);return(<tr key={c.id} style={{borderBottom:`1px solid ${BG3}`}}>
 ...
 <td style={td()}><Bdg l={c.cls.l}/></td>
 <td style={td({color:c.balance>0?R:G,fontWeight:700})}>{f$(c.balance)}</td>
 <td style={tY()}>{c.interest>0?f$(c.interest):"—"}</td>
 <td style={td({color:c.balance>0?R:G,fontWeight:700})}>{f$(c.balance+c.interest)}</td>
 ...
 <div style={{marginTop:18}}><Sec t="Historial de Pagos"/></div>
 <Tbl cols={["Fecha","Contrato","Cliente","Monto","Pts","Método","Costo Tx","Gestor","Estado"]}
 ...
 <td style={tY()}>{f$(p.amount*(p.costPct||0)/100)}</td>
...
function PointSalesView({clients,setClients,payments,setPayments,pp,pms,cu,pendingSales,setPendingSales}){
 ...
 {actPend.length===0?<div style={{color:T4,fontSize:11,textAlign:"center",padding:"18px 0"}}>Sin ventas pendientes.</div>:actPend.map(s=>{const hrs=Math.max(0,Math.round((new Date(s.expiresAt).getTime()-now)/3600000));return(<div key={s.id} style={{background:BG3,borderRadius:8,padding:"11px",marginBottom:9,border:`1px solid ${Y}33`}}>
 ...
 :<Tbl cols={["Fecha","Contrato","Cliente","Pts","Total","Tipo","Por","Estado"]} rows={[...pendingSales].reverse().slice(0,15).map(s=>{const exp=s.status==="Pendiente"&&new Date(s.expiresAt).getTime()<=now;return(<tr key={s.id} style={{borderBottom:`1px solid ${BG3}`}}>
...
function ReportsView({clients,reservations,payments,pp,users,role,courtesies,rc}){
 ...
 const filterP=()=>{const now=TODAY;const y=now.getFullYear(),m=now.getMonth();
  ...
  return payments;};
 const filterR=()=>{const now=TODAY;const y=now.getFullYear(),m=now.getMonth();
 ...
 rows={clients.map(c=>{const inv=c.annualPoints*pp,paid=c.paidPoints*pp,n=payments.filter(p=>p.clientId===c.id).length;return(<tr key={c.id} style={{borderBottom:`1px solid ${BG3}`}}>
 <td style={tBs()}>{c.contractNo}</td>
 <td style={tW()}>{c.name}</td>
 ...
 rows={staff.map(u=>{const uP=filteredPayments.filter(p=>p.processedBy===u.username);const uR=filteredReservations.filter(r=>r.processedBy===u.username);const txC2=uP.reduce((a,p)=>a+p.amount*(p.costPct||0)/100,0);const cC=uP.reduce((a,p)=>a+p.amount*(0),0);const cR=uR.length*rc;const tot=(u.salary||0)+txC2+cC+cR;const rev=uP.reduce((a,p)=>a+p.amount,0);const ratio=rev>0?tot/rev:0;return(<tr key={u.id} style={{borderBottom:`1px solid ${BG3}`}}>
 ...
 {(()=>{const total=txCost+commAll+salaryAll;return[["Costo financiero (medios)",txCost,Y],["Comisiones cobros+reservas",commAll,G],["Nómina base",salaryAll,B]].map(([l,v,c])=>{const pctIngr=colTotal>0?(v/colTotal)*100:0;const pctTot=total>0?(v/total)*100:0;return(<div key={l} style={{padding:"6px 0",borderBottom:`1px solid ${BG3}`}}>
 ...
 {[...new Set(filteredPayments.map(p=>p.method))].map(m=>{const mP=filteredPayments.filter(p=>p.method===m),mC=mP.reduce((a,p)=>a+p.amount*(p.costPct||0)/100,0),mA=mP.reduce((a,p)=>a+p.amount,0);const pctIngr=colTotal>0?(mC/colTotal)*100:0;return(<div key={m} style={{padding:"6px 0",borderBottom:`1px solid ${BG3}`}}>
 ...
 rows={clients.map(c=>{const unpaid=Math.max(0,c.annualPoints-c.paidPoints),saved=(c.savedPoints||[]).reduce((a,sp)=>a+sp.points,0);return(<tr key={c.id} style={{borderBottom:`1px solid ${BG3}`}}>
 <td style={tBs()}>{c.contractNo}</td>
 <td style={tW()}>{c.name}</td>
 ...
 <td style={td()}><Bdg l={unpaid>0?"Pendiente":"Liquidado"}/></td>
  ...
  {rep==="ptsAvail"&&<div>
 <div style={{fontSize:11,color:T4,marginBottom:12}}>Clientes con puntos con mantenimiento pagado que no han sido utilizados. Ordenados de mayor a menor cantidad de puntos disponibles.</div>
 {(()=>{const list=clients.filter(c=>c.contractStatus==="Active").map(c=>{const usedP=reservations.filter(r=>r.clientId===c.id&&r.status==="Confirmed").reduce((a,r)=>a+r.pointsUsed,0);const savedA=(c.savedPoints||[]).filter(sp=>new Date(sp.expiryDate)>new Date()).reduce((a,sp)=>a+sp.points,0);const avail=c.paidPoints-usedP+savedA;return{...c,usedP,savedA,avail};}).filter(c=>c.avail>0).sort((a,b)=>b.avail-a.avail);const totalAvail=list.reduce((a,c)=>a+c.avail,0);const totalSaved=list.reduce((a,c)=>a+c.savedA,0);return(<>
 <div style={{display:"flex",gap:9,flexWrap:"wrap",marginBottom:14}}>
 ...
 <Tbl cols={["Contrato","Cliente","Pagados","Usados","Guardados","Disponibles","Gestor","Morosidad"]}
 rows={list.map(c=>{const d=dOvr(c.dueDate);const cl=cDelEx(c.balance,d);return(<tr key={c.id} style={{borderBottom:`1px solid ${BG3}`}}>
 <td style={tBs()}>{c.contractNo}</td>
 <td style={tW()}>{c.name}</td>
 <td style={tG()}>{fP(c.paidPoints)}</td>
...
function ConfigView({seasonOrder,setSeasonOrder,uts,setUts,units,setUnits,pp,setPp}){
 ...
 const emptyUnit=()=>({id:"new",typeId:uts[0]?.id,location:"",name:"",active:true,occ:[],seasons:[
 ...
 {seasonOrder.map((sId,i)=>{const meta=SEASON_META.find(m=>m.id===sId);return(<div key={sId} style={{background:BG3,borderRadius:7,padding:"9px 12px",marginBottom:6,display:"flex",alignItems:"center",gap:10,borderLeft:`3px solid ${meta.color}`}}>
 ...
 {editU.seasons.map(s=>{const meta=SEASON_META.find(m=>m.id===s.id);return(<div key={s.id} style={{background:BG3,border:`1px solid ${meta.color}33`,borderRadius:8,padding:"12px 14px",marginBottom:10,borderLeft:`3px solid ${meta.color}`}}>
 ...
 <Tbl cols={["Unidad","Tipo","Ubicación","Temporadas","Occ.","Estado",""]} rows={units.map(u=>{const ut=uts.find(t=>t.id===u.typeId);return(<tr key={u.id} style={{borderBottom:`1px solid ${BG3}`,opacity:u.active?1:.4}}>
...
function CompView({users,setUsers,payments,reservations,pp,rc,setRc,cr,setCr}){
 ...
 rows={staff.map(u=>{const cP=payments.filter(p=>p.processedBy===u.username).reduce((a,p)=>a+p.amount*(0),0);const cR=reservations.filter(r=>r.processedBy===u.username&&r.status==="Confirmed").length*rc;const tot=(u.salary||0)+cP+cR;return(<tr key={u.id} style={{borderBottom:`1px solid ${BG3}`}}>
 <td style={td()}><div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:18,height:18,borderRadius:5,background:u.color+"20",border:`1px solid ${u.color}33`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,color:u.color,fontWeight:700}}>{u.name[0]}</div><span style={{color:T1,fontWeight:600,fontSize:11}}>{u.name}</span></div></td>
 <td style={td()}><RBdg r={u.role}/></td>
 <td style={td({color:B})}>{f$(u.salary||0)}</td>
...
function TeamView({users,setUsers,clients,setClients,payments,reservations}){
 ...
 rows={gestors.map(g=>{const gP=payments.filter(p=>p.processedBy===g.username),gR=reservations.filter(r=>r.processedBy===g.username);return(<tr key={g.id} style={{borderBottom:`1px solid ${BG3}`,opacity:g.active===false?.4:1}}>
 ...
 {gestors.map(g=>{const gC=clients.filter(c=>c.assignedGestor===g.username);return(<div key={g.id} style={{marginBottom:12}}>
...
function Dashboard({clients,reservations,payments,pp,promises,users}){
 ...
 {[{l:"Al corriente",c:G},{l:"1–30 días",c:"#a3e635"},{l:"31–60 días",c:Y},{l:"61–90 días",c:O},{l:"91–180 días",c:R},{l:"+180 días",c:"#dc2626"}].map(bk=>{const cnt=en.filter(c=>c.cls.l===bk.l).length;return cnt>0?(<div key={bk.l} style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
...
export default function App(){
 ...
 {tab==="clients"&&<ClientsView clients={clients} setClients={setClients} pp={pp} cu={cu} payments={payments} reservations={reservations}/>}
 {tab==="cashier"&&<CashierView clients={clients} payments={payments} setPayments={setPayments} setClients={setClients} pp={pp} pms={pms} cu={cu} users={users} pendingPayments={pendingPayments} setPendingPayments={setPendingPayments} promises={promises} setPromises={setPromises} fxRate={fxRate} setFxRate={setFxRate}/>}
 {tab==="res"&&<ReservationsView clients={clients} reservations={reservations} setReservations={setReservations} units={units} setUnits={setUnits} uts={uts} cu={cu} rc={rc} payments={payments}/>}
 {tab==="col"&&<CollectionsView clients={clients} payments={payments} pp={pp} promises={promises} setPromises={setPromises} cu={cu}/>}
